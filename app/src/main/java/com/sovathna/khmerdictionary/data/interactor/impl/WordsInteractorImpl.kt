...
class WordsInteractorImpl @Inject constructor(
  ...
  override val getWords = ObservableTransformer<WordsIntent.GetWords, WordsResult> {
    it.flatMap {
      repository
        .getWordsPager()
  ...
  override val selectWord = ObservableTransformer<WordsIntent.SelectWord, WordsResult> {
    it.flatMap { intent ->
      repository
