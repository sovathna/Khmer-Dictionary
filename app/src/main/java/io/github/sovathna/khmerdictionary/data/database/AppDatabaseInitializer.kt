  ...
  companion object {
    private val MIGRATION_2_3 = object : Migration(2, 3) {
      override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL(
          "ALTER TABLE dict ADD COLUMN is_select INTEGER NOT NULL DEFAULT 0"
        )
        database.execSQL(
          "ALTER TABLE dict ADD COLUMN is_bookmark INTEGER NOT NULL DEFAULT 0"
        )
        database.execSQL(
          "ALTER TABLE dict ADD COLUMN is_history INTEGER NOT NULL DEFAULT 0"
        )
        database.execSQL(
